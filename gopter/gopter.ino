...
unsigned long loop_timer,tiempoInicio, pwm_timer;
...
int estat, estat_motor_throttle;
unsigned long contador_throttle;
...
float complementaryFilter(float newAngle, float previousAngle, float dt, float alpha)
{
  float filteredAngle = alpha * (previousAngle + dt * newAngle) + (1 - alpha) * previousAngle;
...
void motorThrottle(){
  if(start == 5){
    if(estat_motor_throttle == 0){
        esc_1 = 1000;
        esc_2 = 1000;
        esc_3 = 1000;
        esc_4 = 1000;
        receiver_input_channel_3 = 1000;//convert_receiver_channel(3);                 //Convert the actual receiver signals for throttle to the standard 1000 - 2000us.
        estat_motor_throttle++;
    }else if(estat_motor_throttle == 1){
      if(micros() - contador_throttle >= 100000){
        if(esc_1 < 3000){
          //receiver_input_channel_3 = receiver_input_channel_3 + 10;
          esc_1 = esc_1 + 10;
          esc_2 = esc_2 + 10;
          esc_3 = esc_3 + 10;
          esc_4 = esc_4 + 10;
        ...
        contador_throttle = micros();
      }
    }else if(estat_motor_throttle == 2){
      if(micros() - contador_throttle >= 5000000) {
        estat_motor_throttle++;
        contador_throttle = micros();
      }
    }else if(estat_motor_throttle == 3){
      if(micros() - contador_throttle >= 100000){
        if(esc_1 > 1000){
          esc_1 = esc_1 - 10;
          esc_2 = esc_2 - 10;
    ...
    }else if(estat_motor_throttle == 4){
      esc_1 = 1000;
      esc_2 = 1000;
      esc_3 = 1000;
      esc_4 = 1000;
...
void loop() {
  ...
  if(estat!=2){
    ...
    if(start == 5){
      ...
      gyro_roll -= gyro_x_cal;                                                //Subtract the offset calibration value from the raw gyro_x value
      gyro_pitch -= gyro_y_cal;                                                //Subtract the offset calibration value from the raw gyro_y value
      gyro_yaw -= gyro_z_cal;                                                //Subtract the offset calibration value from the raw gyro_z value
    ...
    if(start == 3){
      ...
      if(aux == 0){
        ...
        if(micros() - tiempoInicio > 7000000){
          start=4;
          tiempoInicio = micros();
        }
      }
    ...
    if(start == 4){
      ...
      if(esc_4 == 1500 && esc_3 == 1500 && esc_2 == 1500 && esc_1 == 1500){
    ...
    /*
    unsigned long temps = micros() - loop_timer;
    if(temps > 4050){
      Serial.print("CAUTION: "); //digitalWrite(12, HIGH);                           //Turn on the LED if the loop time exceeds 4050us.
  ...
  if(estat == 0){
    ...
    if(micros() - pwm_timer >= 20000){
      ...
      timer_channel_1 = esc_1 + pwm_timer;                                      //Calculate the time of the faling edge of the esc-1 pulse.
      timer_channel_2 = esc_2 + pwm_timer;                                      //Calculate the time of the faling edge of the esc-2 pulse.
      timer_channel_3 = esc_3 + pwm_timer;                                      //Calculate the time of the faling edge of the esc-3 pulse.
      timer_channel_4 = esc_4 + pwm_timer;                                      //Calculate the time of the faling edge of the esc-4 pulse.
      estat++;
    ...
    if(timer_channel_1 <= esc_loop_timer)digitalWrite(4, LOW);
    if(timer_channel_2 <= esc_loop_timer)digitalWrite(5, LOW);
    if(timer_channel_3 <= esc_loop_timer)digitalWrite(6, LOW);
    if(timer_channel_4 <= esc_loop_timer)digitalWrite(7, LOW);
    if(timer_channel_1 <= esc_loop_timer && timer_channel_2 <= esc_loop_timer && timer_channel_3 <= esc_loop_timer && timer_channel_4 <= esc_loop_timer){
      estat = 1;
    }
  }
...
void recordRegisters() {
 ...
 // }                                  //Store last two bytes into accelZ
  ...
  receiver_input_channel_1 = 1450;//convert_receiver_channel(1);                 //Convert the actual receiver signals for pitch to the standard 1000 - 2000us.
  receiver_input_channel_2 = 1450;  //convert_receiver_channel(2);                 //Convert the actual receiver signals for roll to the standard 1000 - 2000us.
  receiver_input_channel_4 = 1450;//convert_receiver_channel(4);                 //Convert the actual receiver signals for yaw to the standard 1000 - 2000us.
